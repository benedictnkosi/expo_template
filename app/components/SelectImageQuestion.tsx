...
export function SelectImageQuestion({
    ...
    // Shuffle options on mount and when options change
    React.useEffect(() => {
        if (options.length > 0) {
    ...
    if (
        ...
        correctIndex = shuffledOptions.findIndex((optionId) => String(optionId) === String(options[correctOption]));
    ...
    function resetQuestion() {
        ...
        // Reshuffle options for a fresh order
        if (options.length > 0) {
            const shuffled = [...options].sort(() => Math.random() - 0.5);
            setShuffledOptions(shuffled);
        }
